...
const register = async (req: Request, res: Response): Promise<void> => {
    ...
    try{
        ...
        Logger.error(err);
        res.statusMessage = `ERROR creating user ${firstName} ${lastName}: ${err}`;
        res.status(500).send();
        return;
    }
};
...
const login = async (req: Request, res: Response): Promise<void> => {
    ...
    try{
        ...
        res.status( 500 ).send( `ERROR reading user with email ${email}: ${ err }` );
        return;
    }
}
    ...
    try{
        ...
        if (result.length !== 0) {
            if (req.header("X-Authorization") === result[0].auth_token) {
                res.status(200).send({"firstName": result[0].first_name,
